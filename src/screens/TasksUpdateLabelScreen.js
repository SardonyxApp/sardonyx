...
import { ScrollView, View, Text, TextInput, StyleSheet, Platform, StatusBar } from 'react-native';
...
export default class TasksUpdateLabelScreen extends React.Component {
  ...
  static navigationOptions = ({ navigation }) => {
    return {
      title: navigation.getParam('label')
        ? 'Update ' + navigation.getParam('label').name
        : 'Create new label'
    };
  };
  ...
  _handlePress() {
    ...
    if (this.props.navigation.getParam('label'))
      payload.id = this.props.navigation.getParam('label').id;
      ...
      <ScrollView contentContainerStyle={updateStyles.primaryContainer}>
        <ScrollView contentContainerStyle={updateStyles.secondaryContainer}>
          ...
          <View style={updateStyles.inline}>
            ...
            <View
              style={[updateStyles.dot, { backgroundColor: this.state.color }]}
            ></View>
          </View>
          <Text style={updateStyles.description}>
            Choose a color or type your own
          </Text>
          <ColorPicker
